...
angular.module("vsdatatable", ["template-vsdatatable-0.0.1.html"]).constant("vsdatatableConfig", {
    ...
    link: function (d, f, g) {
      ...
      function n() {
        ...
        return angular.forEach(d.options.columns, function (b) {
          e.isUndefined(b.filter) || e.isUndefined(b.filter.template) || (d.columnFilter[b.filter.match][b.prop] = "", a += "columnFilter." + b.filter.match + "." + b.prop + " + ")
        }), a
      }
      ...
      function w() {
        var a = parseInt(100 / d.options.columns.length - 1);
      ...
      function x() {
        d.extDataPagination = d.options.data.extDataPagination, w(), m()
      }

      d.filterFocus = !1, d.sort = {col: "", reverse: !1}, d.globalFilter = "", d.columnFilter = {
        contain: {},
        exact: {}
      };
      ...
      d.addRow = function () {
        ...
        d.filteredItems = b.data, d.totalCount = b.totalCount, e.isEqual(y, d.config.EXT_BTN) || e.isEqual(y, d.config.OPER_EDIT) || e.paginatorEvent(d)
        ...
        d.extDataPagination && (y = a, (e.isEqual(a, d.config.OPER_ADD) || e.isEqual(a, d.config.OPER_DELETE) || e.isEqual(a, d.config.EXT_FLT)) && (d.paginator.visiblePageIdx = 0), d.options.data.extPaginationOperationCb({
  ...
  return {
    restrict: "A", scope: !1, templateUrl: "templates/vspaginator.html", link: function (b, c, d) {
      function e(c) {
        a.isEqual(c, b.config.PAGINATOR_BTN_FIRST) ? g(0, c) : a.isEqual(c, b.config.PAGINATOR_BTN_BACK) ? a.isEqual(b.paginator.visiblePageIdx - b.paginator.pageFirstIdx, 0) ? g(b.paginator.visiblePageIdx - p, c) : h(b.paginator.visiblePageIdx - 1, b.paginator.pageFirstIdx) : a.isEqual(c, b.config.PAGINATOR_BTN_PREV_SET) ? g(b.paginator.pageFirstIdx - p, c) : a.isEqual(c, b.config.PAGINATOR_BTN_LAST) ? g(b.totalPages - 1, c) : a.isEqual(c, b.config.PAGINATOR_BTN_NEXT) ? a.isEqual(b.paginator.visiblePageIdx - b.paginator.pageFirstIdx, p - 1) ? g(b.paginator.pageFirstIdx + p, c) : h(b.paginator.visiblePageIdx + 1, b.paginator.pageFirstIdx) : a.isEqual(c, b.config.PAGINATOR_BTN_NEXT_SET) && g(b.paginator.pageFirstIdx + p, c)
      }
      ...
      function g(c, d) {
        var e = 0, f = 0;
        if (c > b.paginator.visiblePageIdx)if (e = c, a.isEqual(d, b.config.PAGINATOR_BTN_LAST))f = e - p + 1; else {
          var g = i(c);
          f = a.isEqual(g, c) ? c : g
        } else if (c < b.paginator.visiblePageIdx && !a.isEqual(d, b.config.PAGINATOR_BTN_FIRST)) {
          var g = j(c);
      ...
      function k() {
        ...
        b.paginatorButtons.length = 0, b.options.paginator.allNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_FIRST), b.options.paginator.pageNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_BACK), b.options.paginator.setNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_PREV_SET);
        for (var c = a; p + a > c; c++)b.paginatorButtons.push({id: c + 1, label: c + 1});
        b.options.paginator.setNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_NEXT_SET), b.options.paginator.pageNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_NEXT), b.options.paginator.allNavBtnVisible && b.paginatorButtons.push(b.config.PAGINATOR_BTN_LAST), l()
      }

      function l() {
        b.disabledButtons.length = 0, a.isEqual(b.paginator.visiblePageIdx, 0) && (b.disabledButtons.push(b.config.PAGINATOR_BTN_FIRST), b.disabledButtons.push(b.config.PAGINATOR_BTN_BACK)), a.isEqual(b.paginator.pageFirstIdx, 0) && b.disabledButtons.push(b.config.PAGINATOR_BTN_PREV_SET), b.paginator.pageFirstIdx + p >= b.totalPages && b.disabledButtons.push(b.config.PAGINATOR_BTN_NEXT_SET), b.paginator.visiblePageIdx >= b.totalPages - 1 && (b.disabledButtons.push(b.config.PAGINATOR_BTN_LAST), b.disabledButtons.push(b.config.PAGINATOR_BTN_NEXT))
      }
      ...
      function n() {
        b.pageSizeOptions = b.options.paginator.pageSizeOptions, o = b.options.paginator.buttonCount > b.config.PAGINATOR_MAX_BTN_COUNT ? b.config.PAGINATOR_MAX_BTN_COUNT : b.options.paginator.buttonCount;
        var a = 0;
      ...
      b.pageSizeButtonClick = function (a) {
        ...
        return a.isEqual(c, b.config.PAGINATOR_BTN_FIRST) || a.isEqual(c, b.config.PAGINATOR_BTN_BACK) || a.isEqual(c, b.config.PAGINATOR_BTN_PREV_SET) || a.isEqual(c, b.config.PAGINATOR_BTN_NEXT) || a.isEqual(c, b.config.PAGINATOR_BTN_NEXT_SET) || a.isEqual(c, b.config.PAGINATOR_BTN_LAST)
...
}]).directive("colResizer", ["$compile", "$document", "$timeout", "vsdatatableService", function (a, b, c, d) {
  return {
    restrict: "A", scope: !1, link: function (e, f, g) {
      function h(a) {
        a.preventDefault(), n = a.clientX, o = angular.element(a.target).parent(), p = o.next(), d.isEqual(p.prop("id"), "headerColAction") || (q = o.prop("offsetWidth"), r = p.prop("offsetWidth"), s = f.prop("offsetWidth"), b.on("mousemove", i), b.on("mouseup", j), k("col-resize"))
      }

      function i(a) {
        var b = a.clientX - n, c = q + b, d = r - b;
        b > 0 && d < e.config.COL_RESIZER_MIN_COL_WIDTH || 0 > b && c < e.config.COL_RESIZER_MIN_COL_WIDTH || (o.css("width", c / s * 100 + "%"), p.css("width", d / s * 100 + "%"))
      }

      function j() {
        b.off("mousemove", i), b.off("mouseup", j), k("default")
      }
      ...
      function l(a) {
        a.css("background-clip", "padding-box"), a.css("position", "relative")
      }

      function m() {
        for (var b = "position:absolute;border:1px solid transparent;background-color:transparent;top:0;bottom:0;right:0;width:6px;cursor:col-resize;", c = f[0].querySelectorAll("th"), d = 0; d < c.length - 2; d++) {
          var g = angular.element(c[d]), i = angular.element('<div class="colresizer" ng-click="$event.stopPropagation()" style="' + b + '"></div>');
          i.on("mousedown", h), l(g), g.append(i), a(i)(e)
        }
      }

      var n = 0, o = 0, p = 0, q = 0, r = 0, s = 0;
      e.$on("$destroy", function () {
        for (var a = f[0].querySelectorAll(".colresizer"), b = 0; b < a.length; b++)elements[b].off("mousedown", h)
      }), e.options.columnResize && c(m)
    }
  }
}]);