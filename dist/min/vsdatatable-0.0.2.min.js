...
angular.module("vsdatatable", ["template-vsdatatable-0.0.2.html"]).constant("vsdatatableConfig", {
...
}]).directive("colFilterTemplate", ["$compile", "vsdatatableService", "vsdatatableConfig", function (a, b, c) {
  return {
    restrict: "A", scope: !1, link: function (d, e, f) {
      function g() {
        var g = d.$eval(f.colFilterTemplate);
        if (!b.isUndefined(g.filter) && !b.isUndefined(g.filter.template) && !b.isUndefined(g.filter.match)) {
          var h = angular.copy(g.filter.template);
          h = h.replace(c.COLUMN_PROP_VALUE, "columnFilter" + d.config.DOT_SEPARATOR + g.filter.match + d.config.DOT_SEPARATOR + g.prop + '"');
          var i = angular.element(h);
          a(i)(d), e.append(i)
        }
      }
  ...
  return {
    restrict: "A", scope: !1, link: function (c, d, e) {
      function f() {
        ...
        j = b(function () {
          f.overflow ? d[0].scrollWidth > d[0].offsetWidth && (i = angular.element('<div class="overlay" ng-click="closeOverlay()" >' + f.text + "</div>")) : i = angular.element('<div class="overlay" style="margin-left:' + d.prop("offsetLeft") + 'px;">' + f.text + "</div>"), angular.equals(i, null) || (d.append(i), a(i)(c))
