...
angular.module("vsdatatable", ["template-vsdatatable-0.1.2.html"]).constant("vsdtConf", {
...
}]).factory("vsdtEvent", ["vsdtConf", function (a) {
  var b = {};
  return b.setExtPaginationData = function (b, c, d) {
    b.$broadcast(a.SET_EXT_PAGINATION_DATA_EVENT, {data: c, totalCount: d})
  }, b
}]).service("vsdtServ", ["$http", "$templateCache", "vsdtConf", function (a, b, c) {
  var d = {};
  return d.isUndefined = function (a) {
    return angular.isUndefined(a)
  }, d.isEqual = function (a, b) {
    return angular.equals(a, b)
  }, d.isObject = function (a) {
    return angular.isObject(a)
  }, d.setFilterFocus = function (a) {
    a.$broadcast(c.FILTER_FOCUS_EVENT)
  }, d.paginatorEvent = function (a) {
    a.$broadcast(c.PAGINATOR_EVENT)
  }, d.getTemplate = function (c) {
    var d = a.get(c, {cache: b}).success(function (a) {
      return a.data
    });
    return d
  }, d
}]).directive("vsdatatable", ["$compile", "$templateCache", "vsdtConf", "vsdtServ", function (a, b, c, d) {
  return {
    restrict: "EA",
    templateUrl: "templates/vsdatatable.html",
    scope: {options: "="},
    controller: ["$scope", function (a) {
      a.config = c, a.colInitDone = !1, a.colTogglerShow = !1, a.busyIcon = !1, a.filteredItems = [], a.selectedRows = [], a.totalCount = 0, a.sort = {
        col: "",
        reverse: !1
      }, a.globalFilter = "", a.columnFilter = {contain: {}, exact: {}, daterange: {}}
    }],
    link: function (c, e, f) {
      function g(a) {
        if (d.isEqual(c.options.row.selection, 1) || d.isEqual(c.options.row.selection, 2)) {
          var b = c.selectedRows.indexOf(a);
          d.isEqual(b, -1) || (c.selectedRows.splice(b, 1), c.notifyRowSelect(c.config.ROW_DESELECT, a))
        }
      }

      function h(a) {
        c.sort = {col: "", reverse: !1}, c.resetFilter(a)
      }

      function i() {
        c.filteredItems = c.options.data.items, c.totalCount = c.filteredItems.length, h(!1), d.paginatorEvent(c)
      }
      ...
      function k(e) {
        l(), p = angular.element(b.get("rowExtender.html")), d.isEqual(q.oper, c.config.OPER_ADD) ? e.prepend(p) : e.after(p), a(p)(c)
      }

      function l() {
        d.isEqual(p, null) || (p.remove(), p = null)
      }

      function m(a) {
        d.isUndefined(c.options.data.dataOperationCb) || c.options.data.dataOperationCb(a, q.oper, q.dataOld, d.isEqual(a, c.config.OPER_PHASE_BEGIN) ? {} : q.dataNew)
      }

      function n() {
        c.extDataPagination = c.options.data.extDataPagination, c.extDataPagination || (r = c.$watch("options.data.items.length", i));
        var a = 90 / c.options.columns.length;
        c.visibleColCount = 0, angular.forEach(c.options.columns, function (b) {
          c.visibleColCount = d.isUndefined(b.visible) || b.visible ? c.visibleColCount + 1 : c.visibleColCount, d.isUndefined(b.width) && (b.width = {
            number: a,
            unit: "%"
          })
        })
      ...
      var o = null, p = null, q = {}, r = null;
      c.addRow = function () {
        q = {
          oper: c.config.OPER_ADD,
          dataOld: {},
          dataNew: d.isUndefined(c.options.templates.add.defaultValues) ? {} : angular.copy(c.options.templates.add.defaultValues)
        }, m(c.config.OPER_PHASE_BEGIN), c.template = c.options.templates.add;
        var a = angular.element(e[0].querySelector(".tableRows .tableBody"));
        k(a)
      }, c.editRow = function (a, b) {
        c.checkEvent(a) && (q = {
          oper: c.config.OPER_EDIT,
          dataOld: b,
          dataNew: angular.copy(b)
        }, m(c.config.OPER_PHASE_BEGIN), c.template = c.options.templates.edit, k(j(a)))
      }, c.deleteRow = function (a, b) {
        c.checkEvent(a) && (q = {
          oper: c.config.OPER_DELETE,
          dataOld: b,
          dataNew: {}
        }, m(c.config.OPER_PHASE_BEGIN), c.template = c.options.templates["delete"], k(j(a)))
      }, c.viewRow = function (a, b) {
        c.checkEvent(a) && (q = {
          oper: c.config.OPER_VIEW,
          dataOld: b,
          dataNew: {}
        }, m(c.config.OPER_PHASE_BEGIN), c.template = c.options.templates.view, k(j(a)))
      }, c.acceptClicked = function () {
        l(), m(c.config.OPER_PHASE_END), !c.extDataPagination && d.isEqual(q.oper, c.config.OPER_EDIT) ? c.execFilter() : c.extDataPagination && !d.isEqual(q.oper, c.config.OPER_VIEW) && c.paginationOperation(q.oper), g(q.dataOld)
      }, c.cancelClicked = function () {
        l()
      }, c.notifyRowSelect = function (a, b) {
        d.isUndefined(c.options.row.rowSelectCb) || c.options.row.rowSelectCb(a, b)
      }, c.$on(c.config.SET_EXT_PAGINATION_DATA_EVENT, function (a, b) {
        c.filteredItems = b.data, c.totalCount = b.totalCount, d.isEqual(o, c.config.EXT_BTN) || d.isEqual(o, c.config.OPER_EDIT) || d.isEqual(o, c.config.EXT_SORT) || d.paginatorEvent(c), c.options.busyIcon.visible && (c.busyIcon = !1)
      }), c.getColumns = function () {
        return c.options.columns
      }, c.getOperationDataObject = function () {
        return d.isEqual(q.oper, c.config.OPER_ADD) || d.isEqual(q.oper, c.config.OPER_EDIT) ? {
          oper: q.oper,
          data: q.dataNew
        } : {oper: q.oper, data: q.dataOld}
      }, c.getPropertyValue = function (a, b) {
        if (d.isEqual(b.indexOf(c.config.DOT_SEPARATOR), -1))return a[b];
        var e = b.split(c.config.DOT_SEPARATOR), f = angular.copy(a);
        return angular.forEach(e, function (a) {
          f = f[a]
        }), f
      }, c.getColumnStyle = function (a, b) {
        if (!d.isUndefined(b.rules)) {
          var e = "";
          for (var f in b.rules) {
            var g = c.getPropertyValue(a, b.rules[f].prop), h = angular.copy(b.rules[f].expression.toString());
            if (h = h.split(b.rules[f].prop).join(g.toString()), c.$eval(h)) {
              e = b.rules[f].style;
              break
            }
          }
      ...
      }, c.paginationOperation = function (a) {
        c.extDataPagination && (c.options.busyIcon.visible && (c.busyIcon = !0), o = a, (d.isEqual(a, c.config.OPER_ADD) || d.isEqual(a, c.config.OPER_DELETE) || d.isEqual(a, c.config.EXT_FLT)) && (c.paginator.visiblePageIdx = 0), (d.isEqual(a, c.config.OPER_ADD) || d.isEqual(a, c.config.OPER_DELETE)) && h(!1), c.options.data.extPaginationOperationCb({
          columnFilter: c.columnFilter,
          globalFilter: c.globalFilter,
          page: c.paginator.visiblePageIdx + 1,
          pageSize: c.pageSize.rows,
          sort: c.sort
        }))
      }, c.checkEvent = function (a) {
        return (d.isEqual(a.which, 1) || d.isEqual(a.which, 13)) && !c.busyIcon
      };
      var s = e.on("click", function (a) {
        1 === a.which && c.colTogglerShow && (c.colTogglerShow = !1, c.$apply())
      });
      c.$on("$destroy", function () {
        d.isEqual(r, null) || r(), c.$off(c.config.SET_EXT_PAGINATION_DATA_EVENT), e.off("click", s)
      }), n()
    }
  }
...
}]).directive("colFilterTemplate", ["$compile", "vsdtServ", function (a, b) {
  return {
...
}]).directive("tableBodyRow", ["vsdtServ", function (a) {
  return {
    restrict: "A", scope: !1, link: function (b, c, d) {
      function e(c) {
      ...
      function h() {
        b.options.row.hover && (c.on("mouseenter", f), c.on("mouseleave", g))
      }

      b.rowClicked = function (c, d) {
        if (e(c)) {
...
}]).directive("tablePaginator", ["vsdtServ", function (a) {
  return {
...
}).directive("captionBar", ["$filter", "vsdtServ", function (a, b) {
  return {
    restrict: "A", scope: !1, templateUrl: "templates/vscaption.html", link: function (c, d, e) {
      function f(a) {
        for (var d in c.options.columns)if (b.isEqual(c.options.columns[d].prop, a))return c.options.columns[d].filter.dateFormat
      }

      function g(a, b) {
        b = b.toLowerCase();
        var d = b.indexOf(c.config.YEAR), e = b.indexOf(c.config.MONTH), f = b.indexOf(c.config.DAY);
        return new Date(parseInt(a.substring(d, d + 4)), parseInt(a.substring(e, e + 2)) - 1, parseInt(a.substring(f, f + 2)))
      }
      ...
      function i() {
        b.isEqual(c.sort.col, "") || (c.filteredItems = q(c.filteredItems, c.sort.col, c.sort.reverse))
      }

      function j(a, d) {
        b.isObject(a) && b.isEqual(d.contain, {}) && b.isEqual(d.exact, {}) || (!b.isEqual(a, d) && t && c.execFilterAndSort(), t = !0)
      }

      function k(a, d) {
        !b.isEqual(a, d) && a && b.setFilterFocus(c)
      }

      function l() {
        var a = "";
        return angular.forEach(c.options.columns, function (d) {
          b.isUndefined(d.filter) || b.isUndefined(d.filter.template) || (a += "columnFilter." + d.filter.match + c.config.DOT_SEPARATOR + d.prop + " + ")
        }), a
      }
      ...
      function n() {
        if (c.options.filter.global || c.options.filter.column) {
          var a = l();
          c.options.filter.autoFilter.useAutoFilter && (a += "globalFilter", o = c.$watch(a, j))
        }
        c.options.filter.global && (p = c.$watch("filterFocus", k))
      }

      c.filterFocus = !1;
      var o = null, p = null, q = a("orderBy"), r = a("filter"), s = a("dateRangeFilter"), t = !0;
      c.filterBtnClick = function (a) {
        c.checkEvent(a) && (c.filterFocus = !c.filterFocus, c.filterFocus || c.resetFilter(!0))
      }, c.execFilterAndSort = function () {
        c.extDataPagination ? c.paginationOperation(c.config.EXT_FLT) : (c.execFilter(), i(), b.paginatorEvent(c))
      }, c.resetFilter = function (a) {
        t = a, c.globalFilter = "", m(c.columnFilter.contain), m(c.columnFilter.exact), m(c.columnFilter.daterange), !c.options.filter.autoFilter.useAutoFilter && a && c.execFilterAndSort()
      }, c.sortByCol = function (a, d) {
        a.stopPropagation(), c.checkEvent(a) && (b.isEqual(c.sort.col, d) ? c.sort.reverse = !c.sort.reverse : c.sort.reverse = !1, c.sort.col = d, c.extDataPagination || (b.isEqual(d, "") && c.execFilter(), i()), c.paginationOperation(c.config.EXT_SORT))
      }, c.execFilter = function () {
        c.filteredItems = c.options.data.items, b.isEqual(c.globalFilter, "") || (c.filteredItems = r(c.filteredItems, c.globalFilter));
        var a = h(c.columnFilter.contain);
        b.isEqual(a, {}) || (c.filteredItems = r(c.filteredItems, a));
        var d = h(c.columnFilter.exact);
        b.isEqual(d, {}) || (c.filteredItems = r(c.filteredItems, d, function (a, c) {
          return b.isEqual(a.toString(), c) || b.isEqual(c, "")
        }));
        var e = h(c.columnFilter.daterange);
        b.isEqual(e, {}) || angular.forEach(e, function (a, d) {
          var e = a.split(c.config.DATES_SEPARATOR);
          if (b.isEqual(e.length, 2) && b.isEqual(e[0].length, 10) && b.isEqual(e[1].length, 10)) {
            var h = f(d);
            c.filteredItems = s(c.filteredItems, d, g(e[0], h), g(e[1], h))
          }
        }), c.totalCount = c.filteredItems.length
      }, c.$on("$destroy", function () {
        b.isEqual(o, null) || o(), b.isEqual(p, null) || p()
      }), n()
    }
  }
...
}).directive("overlayWindow", ["$compile", "$timeout", "vsdtServ", function (a, b, c) {
  return {
    restrict: "A", scope: !1, link: function (d, e, f) {
      function g() {
        e[0].scrollWidth > e[0].offsetWidth && (l = b(function () {
          c.getTemplate("datatableoverlaywindow.html").then(function (b) {
            k = angular.element(b.data), k.css("margin-top", "-20px"), k.css("margin-left", "14px"), k.text(f.overlayWindow), e.append(a(k)(d))
          })
        }, d.config.OVERLAY_SHOW_DELAY))
      }

      function h() {
        i(), angular.equals(k, null) || (k.remove(), k = null)
      }
      ...
      function j() {
        d.options.showOverlay && (e.on("mouseenter", g), e.on("mouseleave", h))
      }

      var k = null, l = null;
      d.closeOverlay = function (a) {
        a.stopPropagation(), h()
      }, d.$on("$destroy", function () {
        e.off("mouseenter", g), e.off("mouseleave", h)
      }), j()
    }
  }
}]).directive("vstooltip", ["$compile", "$timeout", "vsdtServ", function (a, b, c) {
  return {
    restrict: "A", scope: !1, link: function (d, e, f) {
      function g() {
        n = b(function () {
          i(), o = b(function () {
            j()
          }, d.config.TOOLTIP_CLOSE_DELAY, !0)
        }, d.config.TOOLTIP_SHOW_DELAY, !0)
      }
      ...
      function i() {
        c.getTemplate("datatabletooltip.html").then(function (b) {
          m = angular.element(b.data), m.css("margin-left", e.prop("offsetLeft") + "px"), m.text(f.vstooltip), e.append(a(m)(d))
        })
      }

      function j() {
        angular.equals(m, null) || (m.remove(), m = null)
      }

      function k() {
        b.cancel(n), b.cancel(o)
      }

      function l() {
        d.options.showTooltips && (e.on("mouseenter", g), e.on("mouseleave", h))
      }
...
}]).directive("colResizer", ["$compile", "$document", "vsdtServ", function (a, b, c) {
  return {
    restrict: "A", scope: !1, link: function (d, e, f) {
      function g(a) {
      ...
      function m() {
        d.options.columnResize && c.getTemplate("datatablecolresizer.html").then(function (b) {
          s = angular.element(b.data), s.on("mousedown", g), e.css("background-clip", "padding-box"), e.css("position", "relative"), e.append(a(s)(d))
        }), d.colInitDone && l()
      }

      var n = 0, o = 0, p = 0, q = 0, r = 0, s = null;
      d.$on("$destroy", function () {
        s.off("mousedown", g), l()
      }), m()
    }
